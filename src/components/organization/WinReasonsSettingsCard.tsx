...
import { Loader2, Plus, Trash2, Pencil, Check, X, Trophy } from 'lucide-react';
import {
  useWinReasons,
  useCreateWinReason,
  useUpdateWinReason,
  useDeleteWinReason,
} from '@/hooks/useWinReasons';
...
const PALETTE = ['#22c55e', '#10b981', '#06b6d4', '#3b82f6', '#8b5cf6', '#ec4899', '#f59e0b', '#f97316', '#ef4444', '#64748b'];

export function WinReasonsSettingsCard() {
  const { data: reasons = [], isLoading } = useWinReasons();
  const create = useCreateWinReason();
  const update = useUpdateWinReason();
  const remove = useDeleteWinReason();
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Cadastre os motivos de ganho que aparecerão como opções no modal de "Venda Realizada" e nos relatórios de Análises.
        </CardDescription>
      </CardHeader>
