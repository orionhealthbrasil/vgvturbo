-- Update get_next_open_slot function to add jitter (0-20 minutes random delay)
...
BEGIN
  ...
  WHILE v_days_checked < 14 LOOP
    ...
    IF v_day_of_week = ANY(v_working_days) THEN
      ...
      IF NOT v_is_holiday THEN
        -- Add random jitter between 0-20 minutes to distribute load
        v_jitter_minutes := floor(random() * 21)::INTEGER;
        
        v_result := (v_check_date + v_open_time + (v_jitter_minutes || ' minutes')::INTERVAL) AT TIME ZONE 'America/Sao_Paulo';
        RETURN v_result;
      END IF;
    END IF;
  ...
  v_jitter_minutes := floor(random() * 21)::INTEGER;
  RETURN (v_check_date + '09:00:00'::TIME + (v_jitter_minutes || ' minutes')::INTERVAL) AT TIME ZONE 'America/Sao_Paulo';
END;
$function$;