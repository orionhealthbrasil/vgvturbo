...
-- Create a function to trigger audit when contact is closed
...
DROP TRIGGER IF EXISTS trigger_audit_on_contact_close ON public.contacts;

-- Create trigger that fires after update (after the sync trigger sets the status)
CREATE TRIGGER trigger_audit_on_contact_close
AFTER UPDATE OF status ON public.contacts
FOR EACH ROW
EXECUTE FUNCTION public.trigger_audit_on_close();

-- Also trigger on insert if created already closed (edge case)
DROP TRIGGER IF EXISTS trigger_audit_on_contact_insert_closed ON public.contacts;
CREATE TRIGGER trigger_audit_on_contact_insert_closed
