...
interface SendMessagePayload {
  ...
  quoted_type?: string;
  // System/automation flag - if true, sent_by_user_id will be null
  is_system_message?: boolean;
  // Per-user preference: if false, do NOT prepend sender name to outbound message
...
async function extractAndCacheOwnerJid(
  ...
  // The Sender field in the response contains our JID (often with a device suffix like :37)
  // Example: "559191807033:37@s.whatsapp.net"
...
async function validateUserOrganization(req: Request, organizationId: string): Promise<{ valid: boolean; userId?: string; error?: string; internal?: boolean }> {
  ...
  if (bearerToken === SUPABASE_SERVICE_ROLE_KEY || internalServiceKey === SUPABASE_SERVICE_ROLE_KEY) {
    return { valid: true, internal: true };
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { organization_id, phone, message, contact_id, client_timestamp, quoted_message_id, quoted_content, quoted_type, is_system_message, include_sender_name } = payload;

    if (!organization_id || !phone || !message || !contact_id) {
      return new Response(
        JSON.stringify({ error: 'organization_id, phone, message, and contact_id are required' }),
    ...
    if (contactData?.is_group && contactData?.group_jid) {
      targetNumber = contactData.group_jid; // e.g., "120363xxxxx@g.us"
      isGroupMessage = true;
      console.log(`Sending to group: ${targetNumber}`);
    }
    
    // Get the sender's profile name (skip for system messages and when user disabled the preference)
    const shouldIncludeSenderName = include_sender_name !== false;
    let senderName: string | null = null;
    if (validation.userId && !is_system_message && shouldIncludeSenderName) {
    ...
    // Stevo API endpoint: POST /send/text
    const sendUrl = `${base_url}/send/text`;
    console.log(`Sending message to ${targetNumber} via instance ${instance_name} from ${senderName || 'unknown'}`);
    ...
    if (quoted_message_id) {
      ...
      // Look up who actually sent the quoted outbound message
      ...
      if (quotedMsg) {
        // Generate media slug lazily for media messages that don't have one yet
        ...
        const displayContent = quotedMsg.message_type === 'image'
        ...
        // Outbound: who actually sent the quoted msg; inbound: contact name or phone
        ...
        const replyLine = (senderName && !is_system_message) ? `*${senderName}:* ${message}` : message;
        requestBody.text = `${prefixLine}\n${replyLine}`;

        if (quotedMsg.whatsapp_message_id) {
          requestBody.quoted = { messageId: quotedMsg.whatsapp_message_id };
        }
        console.log(`Quote prefix added. whatsapp_message_id: ${quotedMsg.whatsapp_message_id}`);
      } else {
        console.log('Quoted message not found, sending without quote');
        requestBody.text = (senderName && !is_system_message) ? `*${senderName}:*\n${message}` : message;
      }
    } else {
      requestBody.text = (senderName && !is_system_message) ? `*${senderName}:*\n${message}` : message;
    }
    ...
    if (!stevoResponse.ok) {
      ...
      if (stevoResponse.status === 404) {
        return new Response(
          JSON.stringify({
            error: 'Endpoint da Stevo não encontrado (404). Verifique a URL base configurada e o caminho do endpoint.',
            details: { url: sendUrl },
          }),
      ...
      return new Response(
        JSON.stringify({ error: 'Falha ao enviar mensagem via WhatsApp' }),
    ...
    // Stevo returns the ID in data.Info.ID or data.key.id
    const whatsappMessageId = stevoData?.data?.Info?.ID || stevoData?.data?.key?.id || stevoData?.key?.id || null;
    console.log('Extracted whatsapp_message_id:', whatsappMessageId);

    // Save message to database (include quote info and sender)
    // Always use server-side time to avoid clock skew with inbound messages
    // (inbound use WhatsApp server time; mixing with browser time causes ordering bugs)
    ...
    const { data: messageData, error: messageError } = await supabaseAdmin
      ...
      .insert({
        ...
        sent_by_user_id: is_system_message ? null : (validation.userId || null),
      })
      .select('id')
      .single();
    ...
    console.error('Error in stevo-send-message:', error);
    return new Response(
